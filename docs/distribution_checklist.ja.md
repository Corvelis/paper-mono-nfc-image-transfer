...
タグ付きの`.github/workflows/release.yml`は、タグと`VERSION`の一致確認、対応
コミット入りマニフェスト、`SHA256SUMS`、プロジェクトのライセンス資料ZIPを生成
します。これは各依存元が要求する完全なライセンス文、対応ソース提供、ストア表示の
最終監査を自動的に代替するものではありません。公開前に
[メンテナー向けリリース手順](releasing.ja.md)と本チェックリストを両方確認します。

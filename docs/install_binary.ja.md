...
この方法はNVSとLittleFSを書き換えないため、受信画像、歩数履歴、歩数目標、
...
2. 最初に`SYNC CLOCK`を選び、スマートフォンからUTC時刻とUTCオフセットを送る。
3. 再度メニューから`RECEIVE IMAGE`を選び、アプリで画像を選択して送る。
4. 転送完了表示が出るまで、スマートフォンのNFCアンテナ位置をPaper Monoへ
   当てたままにする。

画像を初期状態へ戻すときはメニューの`RESET IMAGE`を選びます。歩数履歴、目標、
...
- NFCが始まらない: Paper Mono側で先に`SYNC CLOCK`または`RECEIVE IMAGE`を開き、
  スマートフォンのNFCを有効にする。

...
struct StepDayRecord {
  ...
  uint32_t steps = 0;
  uint32_t goalSteps = PAPER_MONO_DEFAULT_STEP_GOAL;
};
...
public:
  ...
  uint32_t goalSteps() const;
  bool setGoalSteps(uint32_t goalSteps, unsigned long now);
  void saveNow(unsigned long now);
...
private:
  struct Detector {
    ...
    uint32_t diagStartedMs = 0;
    uint32_t diagSamples = 0;
    uint16_t diagCandidates = 0;
    uint16_t diagAcceptedSteps = 0;
    uint16_t diagInvalidSamples = 0;
    uint32_t diagMaxSampleGapMs = 0;
#endif
  };
  ...
  uint32_t configuredGoalSteps_ = PAPER_MONO_DEFAULT_STEP_GOAL;
